...
class Harvester(object):
  ...
  def get_indices(self):
    ...
    response = requests.get(BASE_URL + '/content*/_search?scroll=5m&pretty=true',
                headers=HEADERS)
  ...
  def download_data_from_period(self, start_date, end_date, consume_data, **kwargs):
    ...
    url = '{0}/{1}/{2}'.format(BASE_URL, 'content_*', '_search?scroll=1m')
    #print(url)
...
class ArticleHarvester(Harvester):
  """ A class for harvesting articles from Spinn3r. """
  def __init__(self):
    super().__init__()

  def harvest_data(self, start_date, end_date, domain):
    """ Download and persist the data from a given timespan. """
    data_filename = 'harvest3r_{2}_data_{0}_to_{1}'.format(
      start_date.strftime('%d-%m'),
      end_date.strftime('%d-%m'),
      domain
    )
  ...
  def construct_query(self, **kwargs):
    """ Build a query that gets articles from a specific domain. """
...
if __name__ == '__main__':
  ...
  for harvester in harvesters:
    total_downloaded = harvester.harvest_data(start_date, end_date, sys.argv[4])
